...
parser.add_argument("--trace_file",
                    help="Path to the trace file. Default: trace.txt")
...
if options.trace_file:
    from m5.objects import SimpleTrace  # 确保导入 SimpleTrace
    tracer = SimpleTrace()
    tracer.traceFile = options.trace_file
    tracer.flags = SimpleTrace.Iflags.Instructions
    system.cpu.tracer = tracer

# 这些行确保系统正确运行
# 连接PIO和中断端口到内存总线是x86特有的需求
